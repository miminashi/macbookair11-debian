...
コンテキストとコストの節約のため、**委譲できる作業は Agent ツールでサブエージェントに委譲する**。
モデルはタスクの難度に応じて `model` パラメータで指定する:

| タスクの種類 | 委譲先 |
|---|---|
| コード・ログ・ドキュメントの探索/検索 (結論だけ要るもの) | `model: sonnet` の Explore / general-purpose |
| 定型的な実装、機械的な編集、レポートの下書き、実機での定型的な情報収集 | `model: sonnet` |
| 設計検討、複雑な実装、難しいデバッグの委譲 | `model: opus` |

- 本体 (メインセッション) は方針判断・結果の検証・ユーザとの対話・最終レポートの執筆に集中する
- 独立した調査は複数エージェントを**並列**で起動する (1 メッセージに複数の Agent 呼び出しをまとめる)
- 委譲しないほうがよいもの:
  - 1〜2 コマンドで済む単発の確認
  - 実機の状態を変更する操作 (suspend 実験・設定変更・再起動など。誤操作時の影響が大きいため本体が直接実行する)
- サブエージェントの報告は鵜呑みにせず、重要な結論は本体が裏取りする

## 調査用ソースコード (`src/`)

実機の挙動を**コードレベルで**理解するため、関連 OSS のソースをリポジトリ内の
`src/` ディレクトリに clone して参照する。

- clone 物は `.gitignore` (`/src/`) で除外しており **コミットしない**（再取得可能・巨大なため）
- **必要なソースを必要なタイミングで** clone する（全リポジトリを事前 clone しない）
- 配置は `src/` 直下を目安にする。展開物の形は取得方法で異なる:
  - 方法 (A) git clone → `src/<name>/`（例: `src/systemd/`）に綺麗に入る
  - 方法 (B) Debian ソース → `src/<name>-<版>/`（例: `src/systemd-257.x/`）＋
    `.dsc`/`.orig.tar.*`/`.debian.tar.*` が `src/` 直下に展開される（`/src/` 全体を
    ignore しているのでこれらも当然コミットされない）
- 取得方法は用途で 2 通りを使い分ける:

### (A) upstream git — 設計・履歴・blame を読む用

最新ベースで読みやすく、`git log` / `git blame` で経緯を追える。差分が実機導入版と
ずれうる点に注意。shallow clone で十分:

```bash
git clone --depth 1 <upstream-url> src/<name>
# 特定タグを読むとき:
git clone --depth 1 --branch <tag> <upstream-url> src/<name>
```

### (B) Debian ソース — 実機の導入版に厳密一致させる用

挙動を厳密に追う（Debian 固有パッチ込みで一致させたい）場合は、まず実機の導入版を
確認し、同一版・同一 Debian パッチのソースを取得する:

```bash
# 1) 実機 (MacBook) の導入版を確認
ssh miminashi@macbookair2015.lan 'dpkg-query -W -f="${Version}\n" systemd'

# 2) 同一版のソースを取得（deb-src 設定済みの trixie 環境で）
cd src && apt-get source systemd=<上で得た版>
```

> 開発機で trixie の `apt-get source` が使えない場合の代替:
> - 実機側で `apt-get source` を実行して成果物を scp で持ち帰る、または
> - `https://sources.debian.org/` / `snapshot.debian.org` から該当版を取得する

### このプロジェクトで参照しうる主要リポジトリ

| 用途 | プロジェクト | upstream (方法 A) | Debian パッケージ (方法 B) |
|---|---|---|---|
| suspend/sleep, logind, sleep.conf | systemd | `https://github.com/systemd/systemd.git` | `systemd` |
| 電源管理 (i915 / applespi / rtc / s2idle) | Linux kernel | `https://git.kernel.org/pub/scm/linux/kernel/git/stable/linux.git` | `linux` |
| Wi-Fi GUI / PMF パッチ | NetworkManager | `https://gitlab.freedesktop.org/NetworkManager/NetworkManager.git` | `network-manager` |
| `wl` ドライバ (DKMS) | broadcom-sta | — (clean な upstream git なし) | `broadcom-sta-dkms`（方法 B のみ） |

> broadcom-sta は Broadcom 配布のバイナリ tarball を Debian が再パッケージしたもので
> 公開 git リポジトリが無いため、方法 B（Debian ソース）でのみ取得する。
> カーネルは巨大なので `--depth 1 --branch v<実機の版に近いタグ>` での shallow clone を
> 推奨。全履歴 clone は避ける。

## レポート作成ルール

- レポートはプロジェクトルート以下の `report` ディレクトリに作成する
- レポートのタイトルは、長くなりすぎないように、平易な日本語で書く（専門用語や細かい識別子はファイル名側で吸収し、本文タイトルは通しで読んで内容が掴めるものにする）
- レポートには冒頭に「概要」セクションを設ける
- 「概要」セクションは、通読できる平易な日本語で書く（箇条書きの羅列ではなく、段落として読める文章にする）
- 「概要」セクションは、長くなりすぎないようにする（5 段落が目安、最大でも 8 段落程度）
- 「概要」セクションでは、細かい事実は省いてもよい（事実の詳細は他のセクションにしっかり書く）
- レポートには日時（分まで）を入れる
...
- **プランファイルの添付（必須）**: プランモードで作業を行った場合、レポート作成時に必ず以下の手順でプランファイルを添付すること:
  ...
  3. レポート本文に `## 添付ファイル` セクションを設け、リンクを記載:
     ...
     ```
- レポートを書き終えた後、そのセッションで判明した事実のうち、レポートに記載すべきだが書き漏らしているものがないか再確認する
- 上記の再確認が済んだら、レポート内の記述に矛盾がないか改めて通読して点検する
...
社内の共有ファイルは長らく単一 NAS 上に置かれており、この機体が落ちるとチーム全員の作業が止まるという弱点を抱えていた。前四半期に一度短時間の停止があって以来、冗長性のある分散ストレージへの移行が優先課題として上がっていた。ただし新規ハードウェアを買う余力はなく、既存資産を組み替えて実現できないかを探る必要があった。

今回はその選択肢として、既に運用している Proxmox VE の 3 ノードクラスタに相乗りする形で Ceph を立て、共有ファイル置き場として使えるかを検証した。ストレージノードは新調せず既存の Supermicro をそのまま流用し、ネットワークも新規配線を避けて既存 10GbE 上でパブリック網とクラスタ網を論理分離する構成とした。

構築は Proxmox 同梱の `pveceph` を使ってモニタ・マネージャ・OSD の順に立ち上げ、`rados bench` で replica=2 と replica=3 の両構成の性能を計測した。その結果、replica=3 でも 4K ランダム書き込みは事前見積もり内に収まり、共有ファイルの用途としては十分な性能が出ることが確認できた。運用上の最大の懸念だった「OSD 1 台落ち時のリバランスがクライアント I/O を潰さないか」も、実際に 1 台停止させて計測したところ、体感できるほどの劣化は出なかった。

一方で、10GbE を Proxmox の VM トラフィックと共用している構成が、業務時間帯のピーク負荷でどこまで持ちこたえるかは、今回の短時間計測では見えていない。VM 側のバースト I/O と重なった場合の挙動は、本番投入後にしばらく観測し続ける必要がある。

当面は Ceph 側の信頼度を確かめる期間として、まずバックアップとアーカイブというセカンダリ用途から使い始め、共有ファイルの本番移行はネットワーク面の観測が済んでから段階的に切り替える運用とする。
