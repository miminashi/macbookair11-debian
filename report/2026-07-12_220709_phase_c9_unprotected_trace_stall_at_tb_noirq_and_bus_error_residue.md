...
副産物として、この機体には「誰も触っていないのに復帰が始まる」無人 wake が存在することが分かり、保護を外した実験中は lid を閉じたまま放置しないという教訓を得た。ハングが「静かな死」に至る最後の瞬間そのものは今回も観測できていないが、ここから先は専用測定器の領域であり、ソフトウェアで取れる証拠は出尽くしたと考える。対策の有効性は今回の結果でも揺るがず、実験後は通常構成 (stock カーネル + 保護あり) へ完全に復帰した。
