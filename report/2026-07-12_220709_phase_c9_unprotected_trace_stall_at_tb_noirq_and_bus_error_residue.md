...
これまでの調査で、このハングは「WiFi ドライバの親にあたる PCIe ポートがスリープ中に省電力状態へ落ちること」が引き金であり、WiFi をオフにしたときだけドライバの復帰処理が途中までしか走らない「短縮経路」になることまで分かっていた。残っていた最後の疑問は、その短縮経路が実際にバス上で何を壊しているのかである。これを見るには保護を外してわざとハングさせるしかないため、今回はユーザがそのリスクを引き受ける判断をし、保護を一時的に外した状態でハングの現場を観測した。保護の仕組み (udev rule) 自体は温存し、再起動すれば必ず保護に戻る形で行った。

ハングは 3 回起き、3 回とも停止点の記録が取れた。1 回は以前から知られていた署名 B (グラフィックスドライバの復帰中)。残り 2 回は署名 A で、止まった場所が Thunderbolt ポートの復帰処理であることが今回初めて実名で特定できた。このうち 1 回は就寝中に誰も触っていないのにハングしており、発見まで 14 時間放置されたことで記録が時計の進行に汚されていたが、汚れ方に規則性があることを利用した補正手法を考案して復元でき、クリーンに取れた 3 回目と同じポートを指すことも確認できた。

もう一つの狙いだったバス上の証拠も取れた。復帰のたびにバスの状態を丸ごと保存する観測系を新設したところ、WiFi オフの cycle ではエラーの痕跡 (訂正可能エラー、失敗した書き込みの記録、親ポートでのリンク断など) が頻繁に残るのに対し、WiFi オンでは一切残らないことが分かった。「WiFi オフの復帰はバスレベルで汚く、それがまれに致命傷になる」という前回までの推定に、初めて物的証拠が付いた形である。

副産物として、この機体には「誰も触っていないのに復帰が始まる」無人 wake が存在することが分かった。これは別の新しい現象ではなく、就寝中に無人でハングした 2 回目の正体である: 停止点の記録が「復帰処理の途中」を示しており、誰も触っていない以上、未知の何かが機械を起こし、その復帰がいつもと同じ機序でハングしたことになる。ここから、保護を外した実験中は lid を閉じたまま放置しないという教訓を得た。ハングが「静かな死」に至る最後の瞬間そのものは今回も観測できていないが、ここから先は専用測定器の領域であり、ソフトウェアで取れる証拠は出尽くしたと考える。対策の有効性は今回の結果でも揺るがず、実験後は通常構成 (stock カーネル + 保護あり) へ完全に復帰した。
