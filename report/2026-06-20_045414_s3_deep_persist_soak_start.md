...
- **STH が hibernate まで進むと SLEEP/WAKE が「2 ペア」記録される**。systemd-suspend-then-hibernate は
  ...
  2 組。→ **`s3-soak-report.sh` の sleeps/wakes/STH は実ユーザ睡眠回数を最大 2 倍に過大計上する**。
  ただし true_hang 判定（SLEEP の後に WAKE 無し→次 BOOT）の正しさには影響しない（ペアリングは保たれる）。
  S3 フェーズから電源ボタンで起こす（30min 未到達）と 1 ペアのまま（人手スモーク C-3 が実例, asleep_s=22）。
